...
using UnityEngine;
using System.IO;
public class compress_HeightMap : MonoBehaviour {
    ...
    public void Compress_HeightMap(int NowLevel)
    {
        ...
        int[] fx = new int[4];
        int[] fy = new int[4];
        fx[0] = 0;fx[1] = 0;fx[2] = 1;fx[3] = 1;
        fy[1] = 0;fy[1] = 1;fy[2] = 0;fy[3] = 1;
        for(int i = 0; i < tempLen; i++) 
        {
            for(int j = 0; j < tempLen; j++)
            {
                ...
                for(int k = 0; k < 4; k++)
                {
                    sum += heightmaps[NowLevel + 1][i * 2 + fx[k]][j * 2 + fy[k]];
                }
                float Val = (float)(sum / 4.0f);
                heightmaps[NowLevel][i][j] = Val;
            }
        }
...
效果图展示(我分出来了很多层级，但是因为更高的层级太小了，没有必要展示出来，所以这里只给出4-13层级的结果)：

![](https://i.loli.net/2018/07/30/5b5ef47917a55.png)
![](https://i.loli.net/2018/07/30/5b5ef47922589.png)
![](https://i.loli.net/2018/07/30/5b5ef47918d02.png)
![](https://i.loli.net/2018/07/30/5b5ef479185b6.png)
![](https://i.loli.net/2018/07/30/5b5ef4792b8b1.png)
![](https://i.loli.net/2018/07/30/5b5ef4792ce2d.png)
![](https://i.loli.net/2018/07/30/5b5ef47941891.png)
![](https://i.loli.net/2018/07/30/5b5ef47949ce4.png)
![](https://i.loli.net/2018/07/30/5b5ef4795478d.png)
![](https://i.loli.net/2018/07/30/5b5ef47974803.png)

然后我简略的实现了，点选地形获得图形点信息的功能，并且点选若干个点之后呢，点击那个制作河床按钮之后，可以将高度图进行读取并且进行转换输出。
即可以将高度图进行修改变化，得到一张带有河道的高度图。
...
public class BuildRiverBed : MonoBehaviour {
    ...
    //河道在处理过程中每次下降的高度
    private const float Height_Del=0.002f;
    //河流宽度
    public int AAwidth;
                ...
                for (int iX = tx - AAwidth; iX <= tx + AAwidth; iX++)
                {
                    for (int iZ = tz - AAlength; iZ <= tz + AAlength; iZ++)
                    {
                        if (iX >= 0 && iX < width && iZ >= 0 && iZ < length)
                        {
                            ...
                            Fslope = (heightmaps[iX, iZ] - heightmaps[tz, tx])/ Dis;
                            if (Fslope > Slope_Val)
                            {
...
public class BuildRiverBedEditor : Editor {
    ...
    public override void OnInspectorGUI()
    {
        ...
        scrollPos = EditorGUILayout.BeginScrollView(scrollPos);
        {
            ...
            bed.RiverDepth = EditorGUILayout.Slider("河流深度", bed.RiverDepth, 0f, 10f);
            bed.Slope_Val = EditorGUILayout.Slider("河流坡度", bed.Slope_Val, 0f, 5f);
            bed.AAwidth = EditorGUILayout.IntSlider("处理点周围宽度（河流宽度）", bed.AAwidth, 0, 15);
...
趁着对压缩和解压缩的过程还比较熟悉，不如先搞定这个问题。这几天在解压缩上边稍微费一些心思。

（今天临近下班最后一波操作，再翻译一次Top-Down Pass中的操作细节）

在第一次自下而上的传递之后呢，在离线的压缩中执行此传递。在这种情况下，它计算完全逐步重建输入Ln的压缩分辨率表示所需要的残差E0...n。
在下面的实时解压缩过程中，这是唯一执行的传递。唯一的区别是它不再计算残差，但只是读取他们并且使用他们来重建Mip-Maps。让我们稍微详细
的描述一下吧，来明显一点说明这部分内容是如何实现的。

在之前的描述方法中有概要，我们声称只用一个较小的L-i能够通过几个操作步骤来得到L-i+1；但是这不够真实，它是被我们所简化的。
我们应该预测算子P进行连续的三次。它的形式在这些应用中的每一个都不同，这反映了这样的事实：在每次应用之前，已知不同的高度值。
应用越晚，知道的值就越多。在这些应用中的每一个值之后，在压缩期间计算残差并且在压缩和解压缩期间将残差加到已经计算的值。

然而，确保Li+1和L-i+1之间的最大误差界限保持不变的两个主要原因有：在压缩过程中，每次应用P之后，仍然针对目标mip-Map Li+1计算残差。
并且所有预测都是根据重建值进行的。确保压缩和解压缩都将残差添加到相同的值。因此，让我们解释一下如何执行这三个步骤。
