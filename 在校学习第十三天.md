...
三、接下来根据物理公式尝试将水流流动起来（虽然结果不太好，明天做一哈优化）
...
public class BuildRiverPlus2 : MonoBehaviour
{
    ...
    private void Update()
    {
        if(sum>=6)
        {
            MoveRiver(Time.deltaTime);
            DrawMoveRiver();
            tempflag--;
        }
    }
    void MoveRiver(float ttime)
    {
        float[,] TempWater = new float[heightResolution + 4, heightResolution + 4];
        float[,,] F = new float[heightResolution + 4, heightResolution + 4, 10];
        for (int i = 0; i < heightResolution; i++)
        {
            for (int j = 0; j < heightResolution; j++)
            {
                TempWater[i, j] = waterheightmaps[i, j];
                for (int k = 0; k < 8; k++)
                {
                    F[i, j, k] = 0;
                }
        ...
        for (int i = 0; i < heightResolution; i++)
        {
            for (int j = 0; j < heightResolution; j++)
            {
                if (waterheightmaps[i, j] > 0)
                {
                    for (int k = 0; k < 8; k++)
                    {
                        if (i + fx[k] >= 0 && i + fx[k] < heightResolution && j + fz[k] >= 0 && j + fz[k] < heightResolution)
                        {
                            float a = g * ((waterheightmaps[i, j] + heightmaps[i, j]) - (waterheightmaps[i + fx[k], j + fz[k]] + heightmaps[i + fx[k], j + fz[k]]));
                            F[i, j, k] = a * ttime;
                            F[i, j, k] /= 50;
                        }
                    }
        ...
        for (int i = 0; i < heightResolution; i++)
        {
            for (int j = 0; j < heightResolution; j++)
            {
                for (int k = 0; k < 8; k++)
                {
                    if (i - fx[k] >= 0 && i - fx[k] < heightResolution && j - fz[k] >= 0 && j - fz[k] < heightResolution)
                    {
                        TempWater[i, j] += ttime * (F[i - fx[k], j - fz[k], k] - F[i, j, k]);
                    }
                }
            }
        }
        for (int i = 0; i < heightResolution; i++)
        {
            for (int j = 0; j < heightResolution; j++)
            {
                ...
                waterheightmaps[i, j] = TempWater[i, j];
            }
        }
    }
    void DrawMoveRiver()
...
![](https://i.loli.net/2018/06/13/5b20e011a7993.png)
