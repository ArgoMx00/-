1.又浑浑噩噩一天，今天实现的内容是，将点选的点的信息下拉到一个Folder中。对于点选的点，我们将其
进行地形凹陷，构建河床。
2.对于这些点选的点，我们创建一个按钮R，如果按动了这个按钮R ，那么对应我们将其地形恢复并且删除
河床路径点。
3.点击Build River按钮，将整个河流创建出来。
...
public class Build_River_ToolEditor : Editor {
    ...
    public override void OnInspectorGUI()
    {
        ...
        //为了能够实现还原地形的操作，我们一开始应该先得到地形的高度图，而重写函数不会只运行一次
        //所以我们利用flag标记让初始地形得到的时候不会重复覆盖。
        if (flag == 1)
        {
            rivertool.GetHeightmaps();
            flag = 0;
        }
...
public class Build_River_Tool : MonoBehaviour {
    ...
    public void GetPoint(Vector4 P)
    {
        ...
        cnt++;
        FirstUpdateRiver(cnt-1,cnt-1);
        SecondUpdateRiver(cnt-1,cnt-1);
    }
    //删除点并且还原地形
    public void RemovePos(int p)
    {
        ...
        for(int i=tx-AAwidth;i<=tx+AAwidth;i++)
        {
            for(int j=tz-AAlength;j<=tz+AAlength;j++)
            {
                ...
                {
                    heightmaps[j,i] = returnheightmaps[j,i];
                }
            }
        }
        Debug.Log("恢复完成");
    ...
    public void Build()
    {
        ...
        Psort();
        FirstUpdateRiver(0,cnt-1);
        SecondUpdateRiver(0,cnt-1);
    ...
    //初步保证河流高度不可逆
    void FirstUpdateRiver(int l, int r)
    {
        print(RiverDepth);
        print(cnt);
        for (int i = l; i <= r; i++)
    ...
    /********************************************************************/
    void SecondUpdateRiver(int l,int r)
    {
        for (int i = l; i <= r; i++)
        {
            int tx = GetiX(m_vp3River[i]);
