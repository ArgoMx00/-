...
我们说过，顶点着色器最基本的功能就是把模型的顶点坐标从模型空间转换到齐次坐标空间中。
渲染游戏的过程可以理解成是把一个个顶点经过层层处理最终转化到屏幕上的过程，那么本节
我们就来研究这个过程。

一个顶点最开始是在模型空间中定义的，最后他将会变换到屏幕空间中，得到真正的屏幕像素坐标。

①模型空间：如他的名字所暗示的那样，是和某个模型或者说是对象有挂俺的。每个模型都有自己独立
的坐标空间，当它移动或者旋转的时候，模型空间也会跟着它移动或者旋转，把我们自己当成游戏中
的模型的话，当我们在办公室中移动的时候，我们的模型空间也在跟着移动，当我们转身的时候，我们
本身的前后左右方向也在跟着改变。

②世界空间：世界空间是一个特殊的坐标系，因为它建立了我们所关心的最大的空间。一些读者可能会指出
空间可以是无限大的，怎么会有“最大”这一说法呢？这里说的最大指的是一个宏观的概念，也就是说他是
我们所关心的最外层的坐标空间，以我们这个农场游戏为例子，在这个游戏里世界空间指的就是农场，
我们不关心这个农场在什么地方，在这个虚拟的游戏世界里，农场就是最大的概念。

`顶点变换的第一步，就是将顶点坐标从模型空间转换到世界空间中，这个变换通常叫做模型变换。`

③观察空间：观察空间，也被称为摄像机空间，观察空间可以认为是模型空间的一个特殊例子、在所有模型
中有一个非常特殊的模型，就是摄像机。值得注意的是，观察空间是右手坐标系。摄像机的正前方是-z方向。

`顶点变换的第二步，就是将顶点坐标从世界空间变换到观察空间中。这个变换通常叫做观察变换。`

④裁剪空间：顶点接下来要从观察空间转换到裁剪空间中，这个用于变换的矩阵叫做裁剪矩阵，也被称为投影
矩阵。

`顶点变换的第三步，就是将顶点坐标从观察空间变换到裁剪空间中。`

⑤屏幕空间：屏幕空间是一个二维空间，因此，我们必须把顶点从裁剪空间投影到屏幕空间中，来
生成对应的2D坐标，这个过程就不深入探究了。


`顶点变换的最后一步，就是将顶点从裁剪空间中变换到屏幕空间中`

总结起来顶点变换的步骤就是：

首先我们得到一个顶点在模型空间中的信息，为了能够在游戏中展示出来，我们首先要得到世界空间中的点坐标，
所以第一步就是将顶点从模型空间转换到世界空间中，为了在屏幕中展示出来，我们必须找到摄像头所在的空间。
也就是观察空间，所以第二步就是从世界空间转换到观察空间中，那么接下来为了能够得到视觉效果，我们需要
把顶点从观察空间变换到裁剪空间中，最后映射到屏幕上，将顶点从裁剪空间变换到屏幕空间中即可。
