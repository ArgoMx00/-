...
这里需要注意两点：`1.对于所有顶点来讲，它数组必须是按照等大规模开取数组，否则构造不出形状。2.对于绘制的每一个三角形来讲，必须是顺时针构造的。否则只能去背面观察的到。。。。（所以这东西就这么真实吗。。。。）`
</br></br>
...
public class Shape_Update : MonoBehaviour
{
    ...
    void Drawcircle()
    {
        ...
        for(int i=0;i<n-1;i++)
        {
            ...
            array[i * 3 + 1] = i+2;
            array[i * 3 + 2] = i+1;
            if (array[i * 3] > n - 1) array[i * 3] -= 4;
            if (array[i * 3+1] > n - 1) array[i * 3+1] -= 4;
            if (array[i * 3+2] > n - 1) array[i * 3+2] -= 4;
        }
        mesh.vertices = vp3;
        mesh.triangles = array;
    }
