...
完成了三角形的绘制之后，在这里，我们将在顶点结构中包含四个额外的浮点数。他将定义顶点RGBA的颜色。然后我们将在输入布局中添加另一个元素。
它将告诉管道中的IA我们的顶点结构具有的颜色值。之后，我们修改效果文件以包含顶点颜色。在顶点级别指定颜色的时候，光栅化阶段（RS）将为每个
像素在三角形内插颜色。我们仍然不会在像素着色器中进行任何计算，因为RS会为我们进行计算，接下来进行工作内容。

`效果文件（着色器）`

让我们先看一下Effect效果文件，我们创建顶点和像素着色器。首先，我们创建一个新结构。该结构是VS的返回类型，以及PS的输入类型。我们这样做，
我们可以返回多种类型。因为现在我们有一个定点的额位子和颜色。我们需要能够返回的不仅仅是一个位子。我们再VS中所做的就是讲接收到的顶点值传给
管道中的下一个阶段。颜色值最终会到达PS之前到达RS，其中顶点颜色将在三角形表面进行插值，并且在完成颜色的插值计算之后。每个像素将被发送到PS、
我们所做的就是将像素的颜色值传递给要放置在渲染目标上的OM。
...
因为修改了Shader效果文件中的代码，我现在的输入内容多增加了一个颜色。所以我们现在也同时要改变顶点结构。

`顶点结构`

我们修改了Vertex结构，以包含表示顶点颜色的四个浮点值。这里其实没有什么多余的内容好说的。就是在进行一个练习。
让自己熟知顶点结构和着色器之间的关系。
...
现在我们在输入布局数组中添加了一个新的元素。此元素表示将前12个字节后的下一个16个字节发送到VS中的第一个COLOR变量。
您可能认为谈论字节听起来很奇怪，但这就是输入布局的工作原理。第一个元素是我们的位子，其格式为DXGI_FORMAT_R32G32B32_FLOAT
枚举类型。他为R,G,B各保留32位或4个字节。那就是12个字节。因此position元素的偏移量将为0.因为他是数组中的第一个元素。
颜色是第二个，位于position元素后边的12个字节。由于颜色使用DXGI_FORMAT_R32G32B32A32_FLOAT枚举类型作为格式。
即16个字节。所以如果我们要在输入布局中添加另一个元素，则下一个元素必须为第五个参数添加28，因为他将
位于第28个字节之外数组的开头。
...
接下来要做的就是在创建缓冲区之前将颜色值添加到顶点数组中。我们只做了第一个红色，第二个绿色和第三个蓝色。
这些颜色将在三角形内插值。
...
![](https://i.loli.net/2019/01/20/5c441bfa8559d.png)
